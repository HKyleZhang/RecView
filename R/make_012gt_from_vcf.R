...
#' @usage make_012gt_from_vcf(gt, save_filename)
...
make_012gt_from_vcf <- function(vcf, limit = 1.5e9, F0F1_rename = "no_rename", save_filename = "gt_012.csv", ...) {
  ...
  gt <- vcfR::extract.gt(dd) %>%
    as.data.frame() %>%
    rownames_to_column(var = "id") %>%
    separate(col = "id", into = c("CHROM", "POS"), sep = '[:]', remove = FALSE) %>%
    select(id, CHROM, POS, everything())

  out <- select(gt, id, CHROM, POS)

  gt <- select(gt, -id, -CHROM, -POS)
